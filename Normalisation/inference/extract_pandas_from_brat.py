...
def extract_pandas(IN_BRAT_DIR,
                  OUT_DF = None,
                  labels = None):
    ...
    for ann_file in ann_files:
        ...
        with open(txt_path, 'r', encoding='utf-8') as f_txt:lines_txt = f_txt.readlines()
        patient_num = lines_txt[0][:-1]
        patients.append(patient_num)

        # Read ann file
        with open(ann_path, 'r', encoding='utf-8') as f_in:lines = f_in.readlines()
    ...
    if OUT_DF: dataset_df.to_csv(OUT_DF)
