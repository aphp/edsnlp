...
class PolypharmacyStatusMatcher(FrailtyDomainMatcher):
    """
    The `eds.polypharmacy_status` pipeline component extracts mentions of
    polymedication.
    It is best used in conjuction with the eds.drugs components, since those
    explicit mentions are relatively rare.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/polypharmacy_status/patterns.py"
        # fmt: on
        ```
    ...
    It will specify the severity of the mention regarding the polymedication
    of the patient.\n
    Possible values are:
    ...
    --8<-- "docs/assets/fragments/polypharmacy_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "polypharmacy_status",
        patterns: FullConfig = default_patterns,
        label: str = "polypharmacy_status",
        span_setter: SpanSetterArg = {"ents": True, "polypharmacy_status": True},
    ):
        super().__init__(
            nlp=nlp,
