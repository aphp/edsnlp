...
def train(args, model, train_dataloader, umls_dataset):
    ...
    while True:
        ...
        for _, batch in enumerate(epoch_iterator):
            ...
            loss, (sty_loss, cui_loss, re_loss) = model(
                input_ids_0,
                input_ids_1,
                input_ids_2,
                cui_label_0,
            ...
            writer.add_scalar(
                "rel_count",
                train_dataloader.batch_sampler.rel_sampler_count,
                global_step=global_step,
            )
            writer.add_scalar("batch_loss", batch_loss, global_step=global_step)
            writer.add_scalar("batch_sty_loss", batch_sty_loss, global_step=global_step)
            writer.add_scalar("batch_cui_loss", batch_cui_loss, global_step=global_step)
            writer.add_scalar("batch_re_loss", batch_re_loss, global_step=global_step)
            ...
            epoch_iterator.set_description(
                "Rel_count: %s, Loss: %0.4f, Sty: %0.4f, Cui: %0.4f, Re: %0.4f"
                % (
                    train_dataloader.batch_sampler.rel_sampler_count,
                    batch_loss,
                    batch_sty_loss,
            ...
            if batch_loss < best_batch_loss:
                ...
                if args.use_re:
                    writer.add_embedding(
                        model.re_embedding.weight,
                        metadata=umls_dataset.re2id.keys(),
                        global_step=global_step,
                        tag="re embedding",
                    )
                else:
                    ...
                    writer.add_embedding(
                        model.re_embedding.weight,
                        metadata=umls_dataset.rel2id.keys(),
                        global_step=global_step,
                        tag="rel embedding",
                    )
                ...
                # sty_parameter
                writer.add_embedding(
                    model.linear_sty.weight,
                    metadata=umls_dataset.sty2id.keys(),
                    global_step=global_step,
                    tag="sty weight",
                )
...
def run(args):
    ...
    umls_dataset = UMLSDataset(
        umls_folder=args.umls_dir,
        model_name_or_path=args.model_name_or_path,
        lang=lang,
        json_save_path=args.output_dir,
    )
    ...
    if os.path.exists(args.output_dir):
        ...
        if len(save_list) > 0:
            ...
            if os.path.exists(os.path.join(args.output_dir, "last_model.pth")):
                model = torch.load(os.path.join(args.output_dir, "last_model.pth")).to(
                    args.device
                )
                model_load = True
            else:
                model = torch.load(
                    os.path.join(args.output_dir, f"model_{max(save_list)}.pth")
                ).to(args.device)
                model_load = True
    if not model_load:
        if not os.path.exists(args.output_dir):
            os.makedirs(args.output_dir)
        model = UMLSPretrainedModel(
            device=args.device,
            model_name_or_path=args.model_name_or_path,
...
def main():
    ...
    parser.add_argument(
        "--seed", type=int, default=72, help="random seed for initialization"
    )
    parser.add_argument(
        "--schedule",
        type=str,
    ...
    parser.add_argument(
        "--trans_margin", type=float, default=1.0, help="Margin of TransE."
    )
    parser.add_argument(
        "--use_re", default=False, type=bool, help="Whether to use re or rel."
    )
    parser.add_argument(
        "--num_workers",
        default=1,
    ...
    parser.add_argument(
        ...
        default="eng",
        type=str,
        choices=["eng", "all", "eng_fr"],
        help="language range, eng or all",
    )
    parser.add_argument("--sty_weight", type=float, default=0.0, help="Weight of sty.")
    parser.add_argument("--re_weight", type=float, default=1.0, help="Weight of re.")
