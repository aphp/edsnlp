...
def get_dispatch_schedule(cpu_idx: int, C: range, G: range) -> List[int]:
    """
    ...
    if G:
        if len(G) < len(C):
            r = len(C) % len(G)  # number of CPU without an easy to assign GPU
            R = C[len(C) - r :]
            if cpu_idx not in R:
                base = [G[idx % len(G)]]
                base = base * (len(G) // len(base))
            else:
                base = get_dispatch_schedule(cpu_idx, R, G)
        else:
            idx = C.index(cpu_idx)
            # G = list(range(32))
            r = len(G) % len(C)
            d = len(G) // len(C)
            base = list(G[idx * d : (idx + 1) * d])
            base = base * len(C)  # (len(G) // len(G))
            R = G[-r:]
            if r > 0:
                base = base + get_dispatch_schedule(cpu_idx, C, R)
    return base
...
def execute_multiprocessing_backend(
    ...
    for i in range(len(cpu_workers) + len(gpu_workers)):
