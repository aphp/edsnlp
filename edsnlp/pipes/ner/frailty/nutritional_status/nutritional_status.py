...
class NutritionMatcher(FrailtyDomainMatcher):
    """
    The `eds.nutritional_status` pipeline component extracts mentions of the
    nutritional status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/nutritional_status/patterns.py"
        # fmt: on
        ```
    ...
    It will specify the severity of the mention regarding the nutritional status
    of the patient.\n
    Possible values are:
    ...
    --8<-- "docs/assets/fragments/nutritional_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "nutritional_status",
        patterns: FullConfig = default_patterns,
        label: str = "nutritional_status",
        span_setter: SpanSetterArg = {"ents": True, "nutritional_status": True},
    ):
        super().__init__(
            nlp=nlp,
