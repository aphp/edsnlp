...
class SocialMatcher(FrailtyDomainMatcher):
    """
    The `eds.social_status` pipeline component extracts mentions of the social status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/social_status/patterns.py"
        # fmt: on
        ```
    ...
    - `span._.social_status`: set to None.\n
    ...
    nlp.add_pipe(f"eds.social_status")
    ```

    Below are a few examples:

    --8<-- "docs/assets/fragments/social_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "social_status",
        patterns: FullConfig = default_patterns,
        label: str = "social_status",
        span_setter: SpanSetterArg = {"ents": True, "social_status": True},
    ):
        super().__init__(
            nlp=nlp,
