...
- New `eds.doc_classifier` trainable component to predict an attribute of a whole document, such as its type or a diagnosis. It is configured with one *head* per predicted attribute: [`eds.single_label_head`](https://aphp.github.io/edsnlp/latest/pipes/trainable/doc-classifier/#edsnlp.pipes.trainable.doc_classifier.heads.SingleLabelHead) when a document carries exactly one label (cross-entropy or focal loss, `argmax` decoding), [`eds.multi_label_head`](https://aphp.github.io/edsnlp/latest/pipes/trainable/doc-classifier/#edsnlp.pipes.trainable.doc_classifier.heads.MultiLabelHead) when it carries a set of them (`BCEWithLogitsLoss`, decoded by thresholding the probabilities). Several heads can share a single pooled document embedding to predict several attributes at once, and a document that lacks a gold value for a head simply does not supervise it, so partially labelled corpora can be mixed in the same training run
...
- New [`eds.doc_classification`](https://aphp.github.io/edsnlp/latest/metrics/doc-classification/) metric to score document-level classification, for one or several `Doc._` attributes at a time. Attributes holding a single value are scored with per-label, micro and macro P-R-F1 (micro then amounting to the accuracy); attributes holding a list of values are scored as a multi-label task. Documents with no gold value for an attribute are left out of its score
- `eds.transformer` now also returns the embedding of the first wordpiece of each context under the `cls` key
- New [Training a document classifier](https://aphp.github.io/edsnlp/latest/tutorials/training-doc-classifier/) tutorial, covering a single-label head (a document type classifier), a multi-label head (the diagnoses associated with a hospital stay) and a two-head model predicting both a principal diagnosis and its associated ones over a shared embedding
