...
Trainable components are preprocessed independently, except for shared weights. In particular, the training sample consumed by one component is not built from the outputs of upstream components in the pipeline, whether these upstream components are  trainable or not. The pipeline still describes how annotations will be produced *at inference time*, but during training, we require that training inputs are provided explicitly in the training data, either from the source (e.g. the base annotated dataset), or from mapped component  (e.g. `training_data.map(my_component)`). This is why a span classifier must be trained on explicit input spans rather than on spans predicted by an upstream NER component. The same rule applies to auxiliary annotations such as sentences or sections when they are used to compute features.
