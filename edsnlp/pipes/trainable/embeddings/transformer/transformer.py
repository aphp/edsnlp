...
class Transformer(WordEmbeddingComponent[TransformerBatchInput]):
    ...
    values are 128 and 96 respectively, which means that the model will process windows
    of 128 tokens, each separated by 96 tokens. Whenever a token appears in multiple
    ...
    # Refolding groups the embeddings by sample
    print(res["embeddings"].refold("sample", "word").shape)
    # Out: torch.Size([2, 10, 32])
    ```
    ...
    window: int
        ...
        before feeding them to the Transformer model (default: 128)
    stride: int
