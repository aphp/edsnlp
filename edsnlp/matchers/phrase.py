...
class FlashTextMatcher(object):
    ...
    def __init__(
        ...
        self.matcher = FTMatcher(vocab, validate=True)
        self.attr = attr
        self.ignore_excluded = ignore_excluded
    ...
    def __call__(
        ...
        """
        if len(self.matcher):
            for match in self.matcher(doclike, as_spans=as_spans, max_cost=self.max_cost):
                yield match
...
## Replace the matcher from spaCy

from spacy.compat import Literal
from edsnlp.matchers.regex import create_span
from edsnlp.utils.flashtext import KeywordProcessor

class FTMatcher(Matcher) :
    def __call__(
        self,
        doclike: Union[Doc, Span],
        *,
        ...
        list_keywords = [] 
        for truc in self._patterns :
            machin = self._patterns[truc]
            for bidule in machin :
                list_keywords.append(bidule[0]['TEXT'])
        
        keyword_processor = KeywordProcessor()
        keyword_processor.add_keywords_from_list(list_keywords)
        res = keyword_processor.extract_keywords(doclike.text, max_cost = max_cost, span_info=True)

        matches = []
        for (word, begin, end) in res :
            span = create_span(
                    doclike=doclike,
                    start_char=begin,
                    end_char=end,
