"""`eds.functional_status` pipeline"""
...
class FunctionalStatusMatcher(FrailtyDomainMatcher):
    """
    The `eds.functional_status` pipeline component extracts mentions
    of the functional status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/functional_status/patterns.py"
        # fmt: on
        ```
    ...
    - `span._.functional_status`: set to None.\n
    It will specify the severity of the mention regarding the functional status
    of the patient.\n
    Possible values are:
    ...
    nlp.add_pipe(f"eds.functional_status")
    ```

    Below are a few examples:

    --8<-- "docs/assets/fragments/functional_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "functional_status",
        patterns: FullConfig = default_patterns,
        label: str = "functional_status",
        span_setter: SpanSetterArg = {"ents": True, "functional_status": True},
    ):
        super().__init__(
            nlp=nlp,
