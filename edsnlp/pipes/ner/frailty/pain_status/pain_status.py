...
class PainMatcher(FrailtyDomainMatcher):
    ...
    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/pain_status/patterns.py"
        # fmt: on
        ```
    ...
    It will specify the severity of the mention regarding the pain status
    of the patient.\n
    Possible values are:
    ...
    --8<-- "docs/assets/fragments/pain_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "pain_status",
        patterns: FullConfig = default_patterns,
        label: str = "pain_status",
        span_setter: SpanSetterArg = {"ents": True, "pain_status": True},
    ):
        super().__init__(
            nlp=nlp,
            domain="pain_status",
