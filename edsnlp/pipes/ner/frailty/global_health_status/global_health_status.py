...
class GlobalHealthStatusMatcher(FrailtyDomainMatcher):
    """
    The `eds.global_health_status` pipeline component extracts mentions
    of the global health status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/global_health_status/patterns.py"
        # fmt: on
        ```
    ...
    --8<-- "docs/assets/fragments/global_health_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "global_health_status",
        patterns: FullConfig = default_patterns,
        label: str = "global_health_status",
        span_setter: SpanSetterArg = {"ents": True, "global_health_status": True},
    ):
        super().__init__(
            nlp=nlp,
