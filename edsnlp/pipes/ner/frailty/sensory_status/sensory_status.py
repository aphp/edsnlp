...
class SensoryMatcher(FrailtyDomainMatcher):
    """
    The `eds.sensory_status` pipeline component extracts mentions of the sensory
    status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/sensory_status/patterns.py"
        # fmt: on
        ```
    ...
    It will specify the severity of the mention regarding the sensory status
    of the patient.\n
    Possible values are:
    ...
    --8<-- "docs/assets/fragments/sensory_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "sensory_status",
        patterns: FullConfig = default_patterns,
        label: str = "sensory_status",
        span_setter: SpanSetterArg = {"ents": True, "sensory_status": True},
    ):
        super().__init__(
            nlp=nlp,
