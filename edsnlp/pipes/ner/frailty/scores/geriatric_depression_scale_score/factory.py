...
def gds_severity_assigner(ent: Span):
    ...
    if reference == 30:
        ...
        raise ValueError(
            f"Unkown reference for geriatric_depression_scale_score : {reference}"
        )


@registry.factory.register(
    "eds.geriatric_depression_scale_score",
    assigns=["doc.ents", "doc.spans"],
)
def create_component(
    nlp: PipelineProtocol,
    name: Optional[str] = "geriatric_depression_scale_score",
    *,
    patterns: List[Dict] = default_patterns,
    score_normalization: Union[str, Callable[[Span], Any]] = score_normalization,
    attr: str = "NORM",
    label: str = "geriatric_depression_scale_score",
    span_setter: SpanSetterArg = {
        "ents": True,
        "geriatric_depression_scale_score": True,
        "psychological_status": True,
    },
    include_assigned: bool = True,
    severity_assigner: Callable[[Span], Any] = gds_severity_assigner,
):
    """
    The `eds.geriatric_depression_scale_score` component extracts mentions of
    the geriatric_depression_scale_score (Geriatric Depression Scale) score.
