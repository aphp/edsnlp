    ...
    for eg_idx, eg in enumerate(examples):
        for span in (
            ...
            *(
                span
                for name in (
                    spans_labels if spans_labels is not None else eg.predicted.spans
                )
                for span in eg.predicted.spans.get(name, ())
            ),
        ):
            if labels is None:
                total_pred_spans.add((eg_idx, span.start, span.end, span.label_))
            elif span.label_ in labels:
                if span.label_ not in pred_spans.keys():
                    pred_spans[span.label_] = set()
                if span.label_ not in gold_spans.keys():
                    gold_spans[span.label_] = set()
                pred_spans[span.label_].add((eg_idx, span.start, span.end, span.label_))
                total_pred_spans.add((eg_idx, span.start, span.end, span.label_))
        ...
        for span in (
            ...
            *(
            ...
            if labels is None:
                total_gold_spans.add((eg_idx, span.start, span.end, span.label_))
            elif span.label_ in labels:
                if span.label_ not in pred_spans.keys():
                    pred_spans[span.label_] = set()
                if span.label_ not in gold_spans.keys():
                    gold_spans[span.label_] = set()
                gold_spans[span.label_].add((eg_idx, span.start, span.end, span.label_))
                total_gold_spans.add((eg_idx, span.start, span.end, span.label_))

    results = {}
    tp = len(total_pred_spans & total_gold_spans)
    results["ents_p"] = (
        tp / len(total_pred_spans)
        if total_pred_spans
        else float(tp == len(total_pred_spans))
    )
    results["ents_r"] = (
        tp / len(total_gold_spans)
        if total_gold_spans
        else float(tp == len(total_gold_spans))
    )
    results["ents_f"] = (
        2 * tp / (len(total_pred_spans) + len(total_gold_spans))
        if total_pred_spans or gold_spans
        else float(len(total_pred_spans) == len(total_gold_spans))
    )

    if labels is not None:
        results["ents_per_type"] = {}
        for label in gold_spans.keys():
            tp = len(pred_spans[label] & gold_spans[label])
            results["ents_per_type"][label] = {
                "p": tp / len(pred_spans[label])
                if pred_spans[label]
                else float(tp == len(pred_spans[label])),
                "r": tp / len(gold_spans[label])
                if gold_spans[label]
                else float(tp == len(gold_spans[label])),
                "f": 2 * tp / (len(pred_spans[label]) + len(gold_spans[label]))
                if pred_spans[label] or gold_spans[label]
                else float(len(pred_spans[label]) == len(gold_spans[label])),
            }
    return results
