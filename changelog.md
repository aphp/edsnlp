...
- New `eds.doc_classifier` trainable component to predict an attribute of a whole document, such as its type or a diagnosis. It is configured with one *head* per predicted attribute: `eds.single_label_head` when a document carries exactly one label (cross-entropy or focal loss, `argmax` decoding), `eds.multi_label_head` when it carries a set of them (`BCEWithLogitsLoss`, decoded by thresholding the probabilities). Several heads can share a single pooled document embedding to predict several attributes at once, and a document that lacks a gold value for a head simply does not supervise it, so partially labelled corpora can be mixed in the same training run
