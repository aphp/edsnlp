...
def parallel_pipe(nlp: Language, 
    ...
    executor = Parallel(n_jobs, backend='multiprocessing', prefer="processes", verbose=verbose)
    ...
    tasks = (do(chunk, **pipe_kwargs) for chunk in chunker(df, len(df), chunksize=chunksize))
