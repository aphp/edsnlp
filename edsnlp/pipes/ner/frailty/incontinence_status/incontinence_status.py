...
class IncontinenceMatcher(FrailtyDomainMatcher):
    """
    The `eds.incontinence_status` pipeline component extracts mentions of the
    incontinence_status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/incontinence_status/patterns.py"
        # fmt: on
        ```
    ...
    --8<-- "docs/assets/fragments/incontinence_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "incontinence_status",
        patterns: FullConfig = default_patterns,
        label: str = "incontinence_status",
        span_setter: SpanSetterArg = {"ents": True, "incontinence_status": True},
    ):
        super().__init__(
            nlp=nlp,
