...
class UMLSPretrainedModel(nn.Module):
    ...
    def forward(
        ...
        pooled_output = self.get_sentence_feature(input_ids)  # (3 * pair) * re_label
        ...
        re_loss = self.re_loss_fn(cui_0_output, cui_1_output, cui_2_output, re_output)
        ...
        if self.standard_dataloader is not None:
            for index, batch in enumerate(self.standard_dataloader):
                ...
                normalized_standard_feature = torch.norm(
                    outputs, p=2, dim=1, keepdim=True
                ).clamp(min=1e-12)
            ...
            assert self.standard_feature.shape == (
                self.num_label,
                self.feature_dim,
            ), self.standard_feature.shape
        return None

    def predict_by_cosine(self, input_ids):
        pooled_output = self.get_sentence_feature(input_ids)

        normalized_feature = torch.norm(pooled_output, p=2, dim=1, keepdim=True).clamp(
            min=1e-12
        )
        normalized_feature = torch.div(pooled_output, normalized_feature)
        sim_mat = torch.matmul(
            normalized_feature, torch.t(self.standard_feature)
