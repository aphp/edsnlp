...
class SensoryMatcher(FrailtyDomainMatcher):
    """
    The `eds.sensory` pipeline component extracts mentions of the sensory status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/sensory/patterns.py"
        # fmt: on
        ```
    ...
    nlp.add_pipe(f"eds.sensory")
    ```

    Below are a few examples:

    --8<-- "docs/assets/fragments/sensory-examples.md"
    ...
    def __init__(
        ...
        *,
        ...
        label: str = "sensory",
        span_setter: SpanSetterArg = {"ents": True, "sensory": True},
    ):
        super().__init__(
            nlp=nlp,
            domain="sensory",
