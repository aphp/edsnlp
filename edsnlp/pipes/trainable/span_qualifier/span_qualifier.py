...
from edsnlp.pipes.trainable.layers.roberta_token_classification import (
...
class TrainableSpanQualifierRoberta(
    TorchComponent[BatchOutput, SpanQualifierBatchInput], BaseComponent
):
    """
    The `eds.span_qualifier` component is a trainable qualifier, predictor of span
        ...
        *,
        ...
        with warnings.catch_warnings():
            ...
            self.classifier = RobertaClassificationHead(
                ...
                hidden_size=self.embedding.output_size,
                num_classes=0,
            )
    ...
    def post_init(self, gold_data: Iterable[Doc], exclude: Set[str]):
        ...
        self.classifier = RobertaClassificationHead(
            dropout=self.dropout,
            hidden_size=self.embedding.output_size,  # FIXME  self.classifier.in_features ????
            num_classes=len(self.bindings),
        )
