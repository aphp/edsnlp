...
class MobilityMatcher(FrailtyDomainMatcher):
    """
    The `eds.mobility_status` pipeline component extracts mentions of the
    mobility status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/mobility_status/patterns.py"
        # fmt: on
        ```
    ...
    It will specify the severity of the mention regarding the mobility status
    of the patient.\n
    Possible values are:
    ...
    --8<-- "docs/assets/fragments/mobility_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "mobility_status",
        patterns: FullConfig = default_patterns,
        label: str = "mobility_status",
        span_setter: SpanSetterArg = {"ents": True, "mobility_status": True},
    ):
        super().__init__(
            nlp=nlp,
