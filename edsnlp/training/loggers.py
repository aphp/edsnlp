...
def AimLogger(
    ...
    **kwargs,
    ...
    logging_dir: Optional[Union[str, os.PathLike]]
        ...
        The environment variable `AIM_LOGGING_DIR` takes precedence over this argument.
    kwargs: Dict
    ...
    if env_logging_dir is not None and logging_dir is not None:  # pragma: no cover
        warnings.warn(
            f"Using the env AIM_LOGGING_DIR={env_logging_dir} as the logging directory"
            f"for Aim, instead of ${logging_dir}."
        )
        logging_dir = env_logging_dir
    assert logging_dir is not None, (
