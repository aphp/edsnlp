...
class PolymedMatcher(FrailtyDomainMatcher):
    """
    The `eds.polymed` pipeline component extracts mentions of polymedication.
    ...
    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/polymed/patterns.py"
        # fmt: on
        ```
    ...
    nlp.add_pipe(f"eds.polymed")
    ```

    Below are a few examples:

    --8<-- "docs/assets/fragments/polymed-examples.md"
    ...
    def __init__(
        ...
        *,
        ...
        label: str = "polymed",
        span_setter: SpanSetterArg = {"ents": True, "polymed": True},
    ):
        super().__init__(
            nlp=nlp,
            domain="polymed",
