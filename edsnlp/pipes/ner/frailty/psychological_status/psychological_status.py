...
class PsychologicalStatusMatcher(FrailtyDomainMatcher):
    """
    The `eds.psychological_status` pipeline component extracts mentions of the
    psychological status.

    ??? info "Details of the used patterns"
        ```{ .python .no-check }
        # fmt: off
        --8<-- "edsnlp/pipes/ner/frailty/psychological_status/patterns.py"
        # fmt: on
        ```
    ...
    - `span._.psychological_status`: set to None.\n
    It will specify the severity of the mention regarding the psychological status
    of the patient.\n
    Possible values are:
    ...
    nlp.add_pipe(f"eds.psychological_status")
    ```

    Below are a few examples:

    --8<-- "docs/assets/fragments/psychological_status-examples.md"
    ...
    def __init__(
        ...
        nlp: Optional[PipelineProtocol],
        *,
        name: str = "psychological_status",
        patterns: FullConfig = default_patterns,
        label: str = "psychological_status",
        span_setter: SpanSetterArg = {"ents": True, "psychological_status": True},
    ):
        super().__init__(
            nlp=nlp,
